                    ...
                    {watches.map((watch) => (
                        <article key={watch.id} className={listPageStyles.article}>
                            <div className={listPageStyles.imageContainer}>
                                {watch.img ? (
                                    <img src={watch.img} alt={watch.name} className={listPageStyles.image}
                                        onError={(e) => {
                            ...
                            <div className="mt-3 flex flex-col gap-2">
                                <h3 className={listPageStyles.name}>{watch.name}
                                </h3>
                                ...
                                <div className='flex items-center gap-2 text-sm mt-2'>
                                    <span className={listPageStyles.categoryTag}>{getCategoryLabel(watch)}</span>
                                </div>
                                <div className="flex items-center justify-between mt-3">
                                    <div className={listPageStyles.price}>₨{Number(watch.price).toLocaleString()}</div>

                                    <button onClick={() => handleDelete(watch.id)} className={listPageStyles.deleteButton} disabled={deletingId === watch.id}>
                                        <Trash2 size={16}/>
