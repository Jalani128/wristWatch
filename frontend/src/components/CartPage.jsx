...
// we have to create a productItem where a 
// user can increase decrease the item in the cart also can see the qty.

function CartProduct({ item }) {
  const { increment, decrement, removeItem } = useCart();
  const [localQty, setLocalQty] = useState(item.qty ?? 1);

  const convertedImg = item.img;
  ...
  return (
    <div className={cartPageStyles.cartItemCard}>
      ...
      <div className={cartPageStyles.cartItemContent}>
        ...
        <p className={cartPageStyles.cartItemPrice}>₨{Number(item.price).toLocaleString()}</p>
...
const CartPage = () => {
  ...
  // check if the form is valid before processing payment
  const isFormValid = () => {
    if (
      !name.trim() ||
      !email.trim() ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        // reset the fields are submission of data.
        setName("");
        setEmail("");
        setAddress("");
        setMobile("");
          ...
          <div className={cartPageStyles.mainGrid}>
            <div className={cartPageStyles.leftColumn}>
              <div className={cartPageStyles.formContainer}>
                <h2 className={cartPageStyles.formTitle}>Checkout Details</h2>
                ...
                <form onSubmit={handleSubmit} className={cartPageStyles.form}>
                  ...
                  <textarea value={note}
                    ...
                    placeholder="Message / instructions delivery"
                    className={cartPageStyles.textareaBase}
                    rows={2}
                  ></textarea>
                  <div className={cartPageStyles.formButtonsContainer}>
                    <button
                      ...
                      {submitting ? "Pocessing..." : "Submit Order"}
                    </button>
